...
def num_tasks() -> int:
    ...
    n_proc: int = int(
        os.environ.get("DEGNNA_NUM_TASKS", num_avail if _has_sched_getaffinity else 1)
    )
    assert n_proc > 0
    assert (
        n_proc <= num_avail
    ), f"Asked for more worker tasks DEGNNA_NUM_TASKS={n_proc} than available CPU cores {num_avail}"
    return n_proc
